...
def ensure_docx_imports():
    global Document, Pt, RGBColor, WD_PARAGRAPH_ALIGNMENT, WD_ALIGN_VERTICAL
    global nsdecls, parse_xml, qn

    if Document is not None:
        return

    from docx import Document as DocxDocument
    from docx.enum.table import WD_ALIGN_VERTICAL as DocxAlignVertical
    from docx.enum.text import WD_PARAGRAPH_ALIGNMENT as DocxParagraphAlignment
    ...
    WD_PARAGRAPH_ALIGNMENT = DocxParagraphAlignment
    WD_ALIGN_VERTICAL = DocxAlignVertical
    nsdecls = docx_nsdecls
    parse_xml = docx_parse_xml
    qn = docx_qn


def get_cache_dir(data_root=None):
    data_root = Path(data_root) if data_root is not None else DEFAULT_DATA_ROOT
    return data_root / "cache"


def get_cache_path(name, data_root=None):
    return get_cache_dir(data_root) / f"{name}.pkl"


def load_pickle_cache(name, data_root=None):
    with get_cache_path(name, data_root).open("rb") as f:
        return pickle.load(f)


def save_pickle_cache(name, value, data_root=None):
    cache_dir = get_cache_dir(data_root)
    cache_dir.mkdir(parents=True, exist_ok=True)
    with get_cache_path(name, data_root).open("wb") as f:
...
def cached_or_fetch(
    ...
    cache_path = get_cache_path(name, data_root)
    if use_cache and not refresh_cache and cache_path.exists():
        return load_pickle_cache(name, data_root)
    if cache_only:
        raise FileNotFoundError(
            f"未找到缓存 {cache_path}。请先运行 build_report_cache.py 生成 data/cache/*.pkl。"
        )
    value = fetcher()
    if save_cache:
        save_pickle_cache(name, value, data_root)
    return value
...
def init_report_data(
    ...
    save_cache=True,
    cache_only=True,
    cache_start_date=DEFAULT_CACHE_START_DATE,
):
    ...
    index_return = cached_or_fetch(
        "index_return",
        lambda: get_multiple_index_return(start_date=cache_start_date),
        data_root,
        use_cache,
        refresh_cache,
        save_cache,
    ...
    returns_bundle = cached_or_fetch(
        "fund_returns_bundle",
        lambda: get_fund_returns(cache_start_date),
        data_root,
        use_cache,
        refresh_cache,
        save_cache,
    ...
    returns, _fund_price = returns_bundle
    bench = cached_or_fetch(
        "bench",
        lambda: pd.read_excel(get_benchmark_path(data_root)),
        data_root,
        use_cache,
        refresh_cache,
        save_cache,
    ...
    half_year = cached_or_fetch(
        "half_year",
        lambda: detail_report(
            fund_position(cache_start_date),
            pd.to_datetime(cache_start_date).strftime("%Y-%m-%d"),
        ),
        data_root,
        use_cache,
        refresh_cache,
    ...
    allo = cached_or_fetch(
        "allo",
        lambda: get_allocation(cache_start_date),
        data_root,
        use_cache,
        refresh_cache,
        save_cache,
    ...
    index_category = cached_or_fetch(
        "index_category",
        get_index_category,
        data_root,
        use_cache,
    ...
    result = cached_or_fetch(
        "fund_benchmark_result",
        lambda: build_fund_benchmark_result(index_return, returns, fund_basic, index_category),
        data_root,
        use_cache,
        refresh_cache,
        save_cache,
    ...
    overrides = {
        "022512.OF": "SPBDUS3T INDEX",
        "968130.OF": "SPBDUS3T INDEX",
        "third_foreword": "000300.SH",
        "third_backword": "000300.SH",
        "968163.OF": "SPBDUS3T INDEX",
        "968153.OF": "SPBDUS3T INDEX",
        "3110.HK": "HSHDYI.HI",
    }
    for code, benchmark in overrides.items():
        result.loc[code] = benchmark
    if save_cache:
        save_pickle_cache("fund_benchmark_result", result, data_root)
    abroad_backup = get_abroad_backup(result, index_category)
    if save_cache:
        save_pickle_cache("abroad_backup", abroad_backup, data_root)
    return result
...
def read_backup_mapper(result, index_category, fund_basic):
    ...
    new_fund_mapper.loc['513950'] = "被动权益"
    new_fund_mapper.loc['159519'] = "被动权益"
...
def generate_fof_report(date, fund, result, data_root="."):
    """为给定日期和基金生成FOF报告。"""
    fund_mapper = build_fund_mapper(result, index_category, fund_basic, benches)
...
def parse_args():
    parser = argparse.ArgumentParser(description="生成招商月报 FOF 运作报告。")
    parser.add_argument(
        "date",
        nargs="?",
        default=DEFAULT_REPORT_DATE,
        help=f"报告日期，格式如 2026-03-31。默认 {DEFAULT_REPORT_DATE}。",
    )
    parser.add_argument(
        "fund",
        nargs="?",
        default=DEFAULT_REPORT_FUND,
        help=f"基金目录名称，如 华夏盈泰稳健。默认 {DEFAULT_REPORT_FUND}。",
    )
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        "--refresh-cache",
        action="store_true",
        help="忽略已有 data/cache/*.pkl，重新连库取数并覆盖缓存。",
    )
    parser.add_argument(
        "--allow-fetch",
        action="store_true",
        help="缓存缺失时允许直接连库取数。默认不连库，只读缓存。",
    )
    parser.add_argument(
        "--cache-start-date",
        default=DEFAULT_CACHE_START_DATE,
        help="重新取数时的起始日期，默认 20230101。",
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="不读取也不写入 data/cache/*.pkl，完全按原始方式连库取数。",
    )
...
def main():
    args = parse_args()
    report_result = init_report_data(
        args.data_root,
        use_cache=not args.no_cache,
        refresh_cache=args.refresh_cache,
        save_cache=not args.no_cache,
        cache_only=(not args.allow_fetch and not args.refresh_cache and not args.no_cache),
        cache_start_date=args.cache_start_date,
    )
