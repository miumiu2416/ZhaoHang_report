...
def init_report_data(data_root=None):
    ...
    global allo, index_category, result, fund_wind2, fund_wind2_full
...
def build_domestic_fund_mapper(result, index_category, fund_basic, passive_returns):
    """生成国内版资产映射，并将主动/被动权益拆成境内权益和境外权益。"""
...
def generate_fof_report(date, fund, result, data_root="."):
    """为给定日期和基金生成FOF报告。"""
    fund_mapper = build_domestic_fund_mapper(result, index_category, fund_basic, benches)
...
def parse_args():
    parser = argparse.ArgumentParser(description="生成国内权益拆分版招商月报 FOF 运作报告。")
